...
from app.domain.repository import PushDeliveryRepository, PushTokenRepository
...
class NotificationService:
    ...
    async def dispatch(self, alert: Alert, device: Device) -> DispatchReport:
        tokens = self._push_tokens.list_active(device.id or 0)
        if not tokens:
    ...
    async def _send_with_retry(self, alert: Alert, device: Device, token: str) -> _Outcome:
        ...
        for attempt in range(1, self._max_attempts + 1):
            result = await self._sender.send(token=token, alert=alert, device=device)
