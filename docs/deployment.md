...
**설정 키를 없애는 변경은 Pi의 `.env`를 같이 고쳐야 한다.** 저장소에서 필드를 지워도
Pi에 그 줄이 남아 있으면 `extra="forbid"`가 부팅을 막는다. 배포는 실패하고 롤백된다
(v0.6.7이 `APP_FAKE_*`로 실제로 이렇게 됐다). 순서는 Pi에서 줄을 먼저 지우고, 그다음
태그다. `deploy.sh`는 서비스를 세우기 전에 `.env`를 읽어 보고 어긋나면 멈춘다 —
돌고 있는 서비스를 내리지 않는다.

Actions는 `.env`를 만들지도 읽지도 않는다. Actions가 아는 비밀은 D3의 접속 자격증명뿐이다.
배포 파이프라인이 뚫려도 앱 운영 비밀은 새지 않는다.
