...
노드는 기울기를 보내지 않는다. 와이어 26바이트에 자리가 없다. 서버가 직전 관측과 견줘
분당 변화율을 채우고, 그 값이 앱의 `channel.slope`으로 간다. 20초에 걸쳐 400을 올리면
`slope`은 1200/min 근처에 머물다가 램프가 끝나면 0으로 떨어진다.

노드가 1Hz 링버퍼로 회귀해 얻는 기울기와 같은 값이 아니다. 전송 간격이 굵을수록 무뎌지고,
그 간격 안에서 오르내린 것은 보이지 않는다. 프레임 v2가 노드의 signature를 실어 오면
서버는 그 채널을 건드리지 않는다 — 채우는 것은 빈자리뿐이다.

직전 관측이 offline 판정 임계(heartbeat의 3배, 기본 15분)보다 오래됐으면 채우지 않는다.
그 침묵 동안 값이 무엇을 했는지 모르는 채로 두 점을 이으면 변화율이 아니다.
